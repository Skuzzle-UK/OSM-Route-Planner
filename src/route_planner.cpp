...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
	float h = 0;
  	RouteModel::Node this_node = *node;
	h = this_node.distance(*end_node);
	return h;
}
...
current_node->FindNeighbors();
  for(auto this_node : current_node->neighbors){
      //cout << this_node << "\n";
      if(!this_node->visited){
        this_node->parent = current_node;
        this_node->g_value = current_node->g_value + current_node->distance(*this_node);  // deref to as this_node is mem address - need data at that address
        //cout << this_node->g_value << "\n";
        this_node->h_value = CalculateHValue(this_node);
        //cout << this_node->h_value << "\n";
        this_node->visited = true;
        open_list.push_back(this_node);
      }
    }
}
...
RouteModel::Node *RoutePlanner::NextNode() {
  std::sort(open_list.begin(), open_list.end(), Compare); // create bool Compare in class to do the math on h + g
